  ...
  return (
    ...
    >
      ...
      {paramInfo.map(({ stepId, nodeType, label, schema, values }) => {
        ...
        const sectionLabel = label ?? (total > 1
...
function ParamControl({ stepId, name, desc, value, onUpdate }: ParamControlProps) {
  ...
  const valueStyle = { color: '#fff', minWidth: 38, textAlign: 'right' as const, flexShrink: 0, fontVariantNumeric: 'tabular-nums' as const }
  ...
  switch (desc.type) {
    ...
    case 'vec2': {
      const vec = Array.isArray(value) ? (value as [number, number]) : ([...desc.default] as [number, number])
      return (
        <>
          <div style={rowStyle}>
