 ...
 * Image Preprocessor
 *
 * GPU compute shader pipeline for image data preparation:
 * - RGB → RGBA conversion (adds alpha channel)
 * - Bit depth remapping (10/12-bit → 16-bit for Uint16)
 *
 * All processing happens on GPU. Output is a GPU buffer with
 * 256-byte row alignment, ready for copyBufferToTexture.
 */
...
/**
 * Result of GPU preprocessing — a buffer ready for copyBufferToTexture.
...
export class ImagePreprocessor {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.log('[ImagePreprocessor] Initialized');
  }
  ...
  analyze(image: ImageData): ImageAnalysis {
    ...
    const needsPreprocessing =
      image.channels !== 4 || (dataType === 1 && bitDepth !== 16);
  ...
  async preprocess(
    ...
    if (!this.initialized) {
      throw new Error('ImagePreprocessor not initialized');
    }
