...
import { NODE_SHADER_PORTS, buildUniformData, createNodePipeline, createUniformBuffer } from './pipelines'
...
export class CompiledGraphPass implements MinimalPass {
  ...
  encode(encoder: GPUCommandEncoder, input: GPUTextureView, output: GPUTextureView): void {
    ...
    for (const step of this.plan.steps) {
      ...
      const inputViews = shaderPorts.map((port) => views.get(step.inputs[port] ?? '') ?? this.defaultView!)
