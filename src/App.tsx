...
import { decodeHDRImage } from './core/decode';
import { FileDrop } from './ui/FileDrop';
import { ImageCanvas } from './ui/ImageCanvas';
import { Controls } from './ui/Controls';
import { HDRInfo } from './ui/HDRInfo';
import { detectHDRCapabilities } from './core/hdr-capabilities';
...
function App() {
  ...
  const handleFileLoaded = async (arrayBuffer: ArrayBuffer, name: string) => {
    try {
      setError(null);
      const decodedImage = await decodeHDRImage(arrayBuffer, name);
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0d0d0d', color: '#fff' }}>
      ...
      >
        ...
        <p style={{ margin: '4px 0 0', fontSize: '14px', color: '#888' }}>
          Scene-referred linear HDR image viewer
        </p>
      </header>
